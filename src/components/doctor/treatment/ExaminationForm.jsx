...
const ExaminationForm = ({
  ...
  // Lấy clinicalResultId khi mount
  useEffect(() => {
    const fetchClinicalResultId = async () => {
      try {
        const results = await clinicalResultsAPI.getClinicalResultsByPatient(patientId);
        if (results && results.length > 0 && results[0].id) {
          // setClinicalResultId(results[0].id); // This line is no longer needed
        } else {
          // setClinicalResultId(null); // This line is no longer needed
        }
      } catch (e) {
        // setClinicalResultId(null); // This line is no longer needed
      }
    };
    fetchClinicalResultId();
  }, [patientId]);
