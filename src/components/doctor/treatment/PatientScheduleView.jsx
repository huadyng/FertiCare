...
const PatientScheduleView = ({
  ...
  const loadTreatmentSchedules = async () => {
    try {
      ...
      // 🆕 Xử lý lỗi authentication - cải thiện logic
      if (error.response?.status === 401 || error.response?.status === 403) {
        console.warn("⚠️ [PatientScheduleView] Authentication error, but continuing with empty data");
        
        // 🆕 Kiểm tra xem có phải lỗi permission không
        const errorMessage = error.response?.data?.message?.messageDetail;
        if (errorMessage && errorMessage.includes("không có quyền")) {
          console.log("ℹ️ [PatientScheduleView] Permission denied - this is expected for new patients");
          // Đây là lỗi permission bình thường cho bệnh nhân mới
          setSchedules([]);
          setTreatmentPlanStatus("none");
        } else {
          // Lỗi authentication thực sự
          console.log("ℹ️ [PatientScheduleView] Authentication error - showing empty state");
          setSchedules([]);
          setTreatmentPlanStatus("none");
        }
      } else {
  ...
  const getAvailableStatusOptions = (schedule) => {
    ...
    // 🆕 Chỉ cho phép cập nhật từ 'scheduled' sang 'completed' hoặc 'cancelled'
    if (currentStatus !== "scheduled") {
      return [
        { 
          value: currentStatus, 
          label: getStatusInfo(currentStatus).text,
          disabled: true 
        }
      ];
    }
    
    // Chỉ hiển thị 2 option: completed và cancelled
    const options = [
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status, record) => {
        ...
        const canUpdate = user?.role === "DOCTOR" && status === "scheduled";
        ...
        return (
          <Space>
            ...
            {user?.role === "DOCTOR" && !canUpdate && (
              <Tooltip title="Không thể cập nhật lịch hẹn đã hoàn thành hoặc hủy">
                <Button
                  type="text"
                  size="small"
                  icon={<LockOutlined />}
                  disabled
                  style={{ padding: 0, color: "#ccc" }}
                />
              </Tooltip>
            )}
          </Space>
