...
const TreatmentScheduleForm = ({
  ...
  const loadAvailableTimes = async (date) => {
    ...
    try {
      console.log(
        "🔍 Loading available times for doctor:",
        doctorId,
        "date:",
    ...
    try {
      if (existingSchedule && isEditing) {
        ...
        form.setFieldsValue({
          ...
          preferredTime: existingSchedule.preferredTime
            ? dayjs(existingSchedule.preferredTime, "HH:mm")
            : null,
        });
  ...
  const createPhaseAppointment = async (phase, scheduleData) => {
    try {
      ...
      console.log(
        "🔍 [createPhaseAppointment] Getting available times for:",
        selectedDateStr
      );

      const availableTimesResult = await getDoctorAvailableTimes(
        doctorId,
      ...
      if (!availableTimesResult.success) {
        return {
          success: false,
          message:
            availableTimesResult.message || "Không thể lấy thời gian khả dụng",
        };
      }
      ...
      const scheduledDateTime =
        firstAvailableTime.dateTime ||
        `${selectedDateStr}T${firstAvailableTime.time}`;
      ...
      if (!planId && phase?.phaseId) {
        console.log(
          "🔄 Using phase.phaseId as fallback plan ID:",
          phase.phaseId
        );
      ...
      if (!planId) {
        ...
        return {
          success: false,
          message: "Không tìm thấy thông tin phác đồ điều trị",
        };
      }
      ...
      const appointmentData = {
        ...
        deadline: dayjs(scheduledDateTime)
          .add(1, "day")
    ...
    try {
      ...
      if (!scheduleData.sessions || scheduleData.sessions.length === 0) {
        ...
        if (apiPhases && apiPhases.length > 0) {
          const currentPhase =
            apiPhases.find((p) => p.status === "In Progress") || apiPhases[0];
          if (currentPhase) {
            ...
            console.log(
              "✅ Created default session from current phase:",
              defaultSession
            );
          } else {
      ...
      if (saveResult.success) {
        ...
        if (apiPhases && apiPhases.length > 0) {
          const currentPhase =
            apiPhases.find((p) => p.status === "In Progress") || apiPhases[0];
          if (currentPhase) {
            const appointmentResult = await createPhaseAppointment(
              currentPhase,
              scheduleData
            );
            if (appointmentResult.success) {
              ...
              message.warning(
                "⚠️ Lưu lịch thành công nhưng không thể tạo lịch hẹn: " +
    ...
    <div className="treatment-schedule-form">
      ...
      >
            ...
            extra={
              <Space>
                <Button
                  ...
                  onClick={async () => {
                    const currentPhase =
                      apiPhases.find((p) => p.status === "In Progress") ||
                      apiPhases[0];
                    if (currentPhase) {
                      const scheduleData = {
                      ...
                      const result = await createPhaseAppointment(
                        currentPhase,
                        scheduleData
                      );
                      if (result.success) {
                        message.success(
                          "✅ Đã tạo lịch hẹn cho giai đoạn hiện tại!"
                        );
                      } else {
                        message.error(
                          "❌ Không thể tạo lịch hẹn: " + result.message
                        );
                      }
                    }
                  }}
        ...
        {apiPhases && apiPhases.length > 0 && (
          <Card size="small" style={{ marginBottom: 16 }} extra={null}>
            <Table
              ...
              <Button
                ...
                title={
                  !isFormValid()
                    ...
                    : isTreatmentPlanDisabled()
                    ? `Không thể tạo lịch điều trị cho phác đồ đã ${
                        currentTreatmentPlan?.status === "completed"
                          ? "hoàn thành"
                          : "hủy"
                      }`
